    ...
    public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject)
    {
        ...
        switch (target)
        {
            case BuildTarget.StandaloneWindows:
                {
                    string launcherDir = Path.Combine(Directory.GetCurrentDirectory(),"Assets","Editor","OtherEditor", "LauncherExecutable", "x86", "WindowsPlayer.exe");
            ...
            case BuildTarget.StandaloneWindows64:
                {
                    string launcherDir = Path.Combine(Directory.GetCurrentDirectory(),"Assets","Editor","OtherEditor", "LauncherExecutable", "x64", "WindowsPlayer.exe");
                    FileUtil.ReplaceFile(launcherDir, pathToBuiltProject);
                }
                break;
