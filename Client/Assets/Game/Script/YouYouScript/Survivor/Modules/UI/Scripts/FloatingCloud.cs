...
namespace Watermelon
{
    ...
    {
        public static FloatingCloud Instance { get; private set; } = new();
        ...
        public void Init()
        {
            ...
            Debugger.LogError("货币数量===>",currencies.Length);
            if(!currencies.IsNullOrEmpty())
            {
                foreach (var currency in currencies)
                {
                    Currency.FloatingCloudCase floatingCloudCase = currency.FloatingCloud;
                    Debugger.LogError("FloatingCloud初始化=======>", currency.CurrencyType.ToString(), "===",
