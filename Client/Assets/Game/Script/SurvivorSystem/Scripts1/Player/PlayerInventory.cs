...
namespace Watermelon
{
    ...
    public class PlayerInventory
    {
        ...
        public bool HasSpace(ref Resource resource)
        {
            Currency currency = CurrencyController.GetCurrency(resource.currency);
            if (currency != null)
            {
                if (currency.Data.UseInventory)
                {
                    if (currentCapacity + resource.amount > maxCapacity)
                    {
        ...
        public bool TryToAdd(CurrencyType currencyType, int amount, bool adsReward = false)
        {
            ...
            if (currency != null)
            {
                ...
                {
                    Debug.LogError("[Inventory]: For non-inventory currencies use default CurrencyController methods!");
                ...
                if (currency.Data.UseInventory)
                {
                    if (!adsReward)
                    {
                        if (currentCapacity + amount > maxCapacity)
                        {
                            ...
                            {
                                // Add items to inventory
                                CurrencyController.Add(currencyType, availableAmount);
        ...
        public void ClearDev()
        {
            for (int i = 0; i < currencies.Length; i++)
            {
                CurrencyController.Substract(currencies[i].CurrencyType, currencies[i].Amount);
            }
