...
public class ReporterPanel : MonoBehaviour
{
    public class Sample
    {
        public float time;
    ...
    private const int LogBufferLimit = 50; // 缓存的日志条数限制
    ...
    public void Initialize()
    {
        ...
        clearOnNewSceneContent = new GUIContent("", getImage("clearOnSceneLoaded", size.x, size.y),
    ...
    void DrawInfo()
    {
        ...
        GUILayout.Space(size.x);
        GUILayout.Label("Mem Usage Of Graph " + graphMemUsage.ToString("0.000") + " mb", nonStyle,
            GUILayout.Height(size.y));
        GUILayout.Space(size.x);
    ...
    void drawToolBar()
    {
        ...
        if (GUILayout.Button(clearOnNewSceneContent, (clearOnNewSceneLoaded) ? buttonActiveStyle : barStyle,
                GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
        {
            clearOnNewSceneLoaded = !clearOnNewSceneLoaded;
        }

        if (GUILayout.Button(showTimeContent, (showTime) ? buttonActiveStyle : barStyle, GUILayout.Width(size.x * 2),
                GUILayout.Height(size.y * 2)))
        {
            showTime = !showTime;
        }
    ...
    bool isGestureDone()
    {
            ...
            Application.platform == RuntimePlatform.IPhonePlayer)
        {
            ...
            else
            {
                ...
                {
                    Vector2 p = Input.touches[0].position;
                    if (gestureDetector.Count == 0 || (p - gestureDetector[gestureDetector.Count - 1]).magnitude >
                        (Screen.width * 0.8f))
                    {
                        gestureDetector.Add(p);
                    }
    ...
    {
        string logEntry = $"{System.DateTime.Now}: [{log.logType}] {log.condition}\n{log.stacktrace}";
        logBuffer.Add(logEntry);

        // 如果缓存的日志数量超过设定的阈值，批量写入文件
        if (logBuffer.Count >= LogBufferLimit)
        {
            WriteLogsToFile();   // 写入文本格式
            logBuffer.Clear();   // 清空缓存
        }
    }
