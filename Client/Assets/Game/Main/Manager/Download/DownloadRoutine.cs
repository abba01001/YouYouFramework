...
namespace Main
{
    ...
    public class DownloadRoutine
    {
        ...
        //临时处理下载文件
        public async void DownloadGameData(string url, int tryRequestCount,Action onUpdate,Action<string> onComplete)
        {
            int currentRetry = 0;
                ...
                using (UnityWebRequest webRequest = UnityWebRequest.Get(url))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            MainEntry.Log(MainEntry.LogCategory.GameData, "成功拉取云端游戏数据");
