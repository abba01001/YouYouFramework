...
public class AudioManager
{
    ...
    {
        AudioSourcePrefab =
            new GameObject("AudioSource", typeof(AudioSource), typeof(PoolObj)).GetComponent<AudioSource>();
    ...
    public async void PlayBGM(string audioName)
    {
        ...
        AudioClip audioClip =
            await GameEntry.Loader.LoadMainAssetAsync<AudioClip>(CurrBGMEntity.AssetFullPath, BGMSource.gameObject);
        PlayBGM(audioClip, CurrBGMEntity.IsLoop == 1, CurrBGMEntity.Volume, CurrBGMEntity.IsFadeIn == 1,
            CurrBGMEntity.IsFadeOut == 1);
    }
    ...
    {
        Sys_AudioEntity sys_Audio = GameEntry.DataTable.Sys_AudioDBModel.GetEntity(audioName);
        AudioSource helper = GameEntry.Pool.GameObjectPool.Spawn(AudioSourcePrefab.transform, poolId: 2)
            .GetComponent<AudioSource>();
        AudioClip clip =
            await GameEntry.Loader.LoadMainAssetAsync<AudioClip>($"{sys_Audio.AssetFullPath}", helper.gameObject);
    ...
    private void PlayAudio3(AudioSource helper, AudioClip audioClip, float volume = 1, int priority = 128)
    {
        ...
        if (!helper.loop)
        {
            ...
            poolObj.OnDespawn = () => { AudioSourceList.Remove(helper); };
        }
    }
