...
public class AssetBundleLoaderRoutine
{
    ...
    public static AssetBundle LoadAssetBundle(string assetBundlePath)
    {
        ...
        if (isExistsInLocal)
        {
            if (assetBundleInfo.IsEncrypt)
            {
                //可写区加载, 需要解密
                byte[] buffer =
                    IOUtil.GetFileBuffer(string.Format("{0}/{1}", Application.persistentDataPath, assetBundlePath));
                if (buffer != null)
                {
                    buffer = SecurityUtil.Xor(buffer);
            ...
            {
                //可写区加载, 不用解密
                return AssetBundle.LoadFromFile(string.Format("{0}/{1}", Application.persistentDataPath,
                    assetBundlePath));
            }
        }
    ...
    {
        AssetBundleLoaderRoutine assetBundleLoaderRoutine =
            MainEntry.ClassObjectPool.Dequeue<AssetBundleLoaderRoutine>();
