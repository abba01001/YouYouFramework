...
public class LoaderManager
{
    ...
    /// 加载资源包
    /// </summary>
    public void LoadAssetBundleAction(string assetbundlePath, Action<AssetBundle> onComplete = null,
        Action<float> onUpdate = null, Action<float> onDownloadUpdate = null)
    {
    ...
    public UniTask<AssetBundle> LoadAssetBundleAsync(string assetbundlePath, Action<float> onUpdate = null,
        Action<float> onDownloadUpdate = null)
    {
        var task = new UniTaskCompletionSource<AssetBundle>();
        LoadAssetBundleAction(assetbundlePath, (AssetBundle bundle) => { task.TrySetResult(bundle); }, onUpdate,
            onDownloadUpdate);
        return task.Task;
    }
    ...
    /// 加载主资源包和依赖资源包
    /// </summary>
    public async UniTask<AssetBundle> LoadAssetBundleMainAndDependAsync(string assetFullName,
        Action<float> onUpdate = null, Action<float> onDownloadUpdate = null)
    {
        ...
        //加载主资源包
        AssetBundle m_MainAssetBundle =
            await GameEntry.Loader.LoadAssetBundleAsync(assetEntity.AssetBundleFullPath, onUpdate, onDownloadUpdate);
    ...
    /// 异步加载
    /// </summary>
    public void LoadAssetAction(string assetFullPath, AssetBundle assetBundle, Action<float> onUpdate = null,
        Action<AssetReferenceEntity> onComplete = null)
    {
        //使用TaskGroup, 加入异步加载队列, 防止高并发导致的重复加载
        AssetTaskGroup.AddTask((taskRoutine) =>
        {
            ...
            if (referenceEntity != null)
            {
                ...
                //GameEntry.LogError("从分类资源池加载" + assetEntity.ResourceName);
                return;
            }
    ...
    public UniTask<AssetReferenceEntity> LoadAssetAsync(string assetFullPath, AssetBundle assetBundle,
        Action<float> onUpdate = null)
    {
        var task = new UniTaskCompletionSource<AssetReferenceEntity>();
        LoadAssetAction(assetFullPath, assetBundle, onUpdate,
            (AssetReferenceEntity assetReferenceEntity) => { task.TrySetResult(assetReferenceEntity); });
        return task.Task;
    }
    ...
    public async UniTask<T> LoadMainAssetAsync<T>(string assetFullPath, GameObject target,
        Action<float> onUpdate = null, Action<float> onDownloadUpdate = null) where T : Object
    {
        if (target == null)
        {
    ...
    /// </summary>
    public async UniTask<AssetReferenceEntity> LoadMainAssetAsync(string assetFullPath, Action<float> onUpdate = null,
        Action<float> onDownloadUpdate = null)
    {
        //从分类资源池(AssetPool)中查找主资源
        AssetReferenceEntity referenceEntity = GameEntry.Pool.AssetPool.Spawn(assetFullPath);
        if (referenceEntity != null)
        {
            ...
            referenceEntity =
 AssetReferenceEntity.Create(assetFullPath, UnityEditor.AssetDatabase.LoadAssetAtPath<Object>(assetFullPath));
#else
        //加载主资源包和依赖资源包
        AssetBundle mainAssetBundle =
            await GameEntry.Loader.LoadAssetBundleMainAndDependAsync(assetFullPath, onUpdate, onDownloadUpdate);
