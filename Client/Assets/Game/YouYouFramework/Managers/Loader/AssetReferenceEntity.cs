...
public class AssetReferenceEntity
{
    ...
    public void ReferenceAdd()
    {
        ...
        if (ReferenceCount == 1)
        {
            ...
            AssetBundleReferenceEntity assetBundleEntity =
                GameEntry.Pool.AssetBundlePool.Spawn(assetEntity.AssetBundleFullPath);
            assetBundleEntity.ReferenceAdd();
            for (int i = 0; i < assetEntity.DependsAssetBundleList.Count; i++)
            {
                AssetBundleReferenceEntity dependAssetBundleEntity =
                    GameEntry.Pool.AssetBundlePool.Spawn(assetEntity.DependsAssetBundleList[i]);
