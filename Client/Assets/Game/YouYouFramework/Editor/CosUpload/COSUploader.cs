...
public static class COSUploader
{
    ...
    private static string rootPath =
        @$"{System.IO.Directory.GetParent(Application.dataPath).FullName}\AssetBundles\{Application.version}\Android\";

    private static string localVersionFilePath =
        @$"{System.IO.Directory.GetParent(Application.dataPath).FullName}/AssetBundles/{Application.version}/Android" +
        $"/{YFConstDefine.VersionFileName}";

    private static string cloudVersionFilePath =
    ...
    private static Dictionary<string, VersionFileEntity> m_LocalAssetsVersionDic =
        new Dictionary<string, VersionFileEntity>();
    ...
    public static async Task UploadVersion(string sourceVersion)
    {
        ...
        using (FileStream fileStream =
               new FileStream(localFilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
        {
    ...
    public static async void UploadAssetBundle(string version, string uploadPath, bool _dontCheck)
    {
        ...
        try
        {
            ...
            string baseFolder = Path.Combine(SettingsUtil.ProjectDir, "AssetBundles", version,
                GetPlatformOption(activeBuildTarget));
            var dic = SecurityUtil.GetSecretKeyDic();
            // 递归上传整个文件夹
                   ...
                   new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                catch (Exception ex)
                {
                    failureLog.AppendLine(
                        $"{Path.GetFileName(filePath)}      上传状态：<color=red>失败</color>，错误：{ex.Message}");
                }
        ...
        if (m_LocalAssetsVersionDic.ContainsKey(abName)) //
        {
            ...
            if (needUpload)
            {
                MainEntry.Log(MainEntry.LogCategory.Assets,
                    $"上传资源{abName},云端md5{m_CDNVersionDic[abName].MD5},本地md5{m_LocalAssetsVersionDic[abName].MD5}");
            }
