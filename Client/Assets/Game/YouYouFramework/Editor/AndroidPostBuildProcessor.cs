...
class AndroidPostBuildProcessor : IPostGenerateGradleAndroidProject
{
    ...
    private string privacyEmail = "【输入你的邮箱】";
...
//                  ""应监管要求,请你务必审慎阅读、充分理解“隐私政策”各条款，包括但不限于：为了向你提供精准广告、优化游戏体验等，我们需要收集你的设备信息等个人信息。\n"" +
                    ...
                    "" 使用者：TopOn SDK, 字节跳动SDK，Gromore SDK，优量汇SDK，快手联盟SDK，友盟SDK\n"" +
                    ...
                    ""\n"" +
                    "" 数据类型：标识符：设备信息（IMEI/MAC/Android ID/IDFA/OpenUDID/GUID/SIM卡IMSI/ICCID/地理位置等）\n"" +
                    "" 使用者：TopOn SDK, 字节跳动SDK，Gromore SDK，优量汇SDK，快手联盟SDK，友盟+SDK，com.unity3d.player,TopOnSDK,com.anythink.china,字节跳动 SDK,pangle广告 sdk\n"" +
                    ...
                    "" 使用者：TopOn SDK, 字节跳动SDK，Gromore SDK，优量汇SDK，快手联盟SDK，TapTap SDK，友盟SDK\n"" +
                    "" 使用目的：第三方广告：我们将统计展示 、点击 、转化广告数据，以用于广告主统计投放结果，向您提供帮助调整广告变现策略的服务。\n"" +
                    "" 是否与身份关联：收集数据的主要目的是用于“第三方广告：显示广告”，在获得用户授权的情况下，才与设备ID、标识符相关联。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“第三方广告：显示广告”。\n"" +
                    ""\n"" +
                    "" 数据类型：加速度传感器，磁场传感器，方向传感器，陀螺仪传感器，重力传感器，线性加速度传感器\n"" +
                    "" 使用者：App本身，com.unity3d.player,ms.bz.bd\n"" +
                    "" 使用目的：App功能：我们将收集运行过程中的传感器信息。\n"" +
                    "" 是否与身份关联：收集数据的主要目的是用于“App功能：确保App正常运行、提升可扩展性和交互体验”，在获得用户授权的情况下，才与设备ID相关联。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“App功能：确保App正常运行、提升可扩展性和交互体验”。\n"" +
                    ""\n"" +
                    "" 数据类型：麦克风录音\n"" +
                    "" 使用者：App本身\n"" +
                    "" 使用目的：App功能：我们将收集声控模式运行过程中的麦克风数据。\n"" +
                    "" 是否与身份关联：收集数据的主要目的是用于“App功能：确保App正常运行、提升可扩展性和交互体验”，在获得用户授权的情况下，才与设备ID相关联。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“App功能：确保App正常运行、提升可扩展性和交互体验”。\n"" +
                    ""\n"" +
                    ...
                    ""\n"" +
                    "" 数据类型：获取应用安装列表\n"" +
                    "" 使用者：TopOn SDK, 字节跳动SDK，Gromore SDK，优量汇SDK，快手联盟SDK，TapTap SDK，友盟SDK，优量汇,yaq.pro.getVresult,yaq.pro.getZresult,TopOnSDK,com.anythink.china,腾讯 SDK,pangle 广告sdk,字节跳动 SDK,yaq.pro.getJresult\n"" +
                    ...
                    "" 用于何种追踪目的：收集数据的主要目的是用于“第三方广告-显示广告”。\n"" +
                    ""\n"" +
                    "" 数据类型：允许写入外置存储器，允许写入SDCard卡数据 \n"" +
                    "" 使用者：TopOn SDK, 字节跳动SDK，Gromore SDK，优量汇SDK，快手联盟SDK，友盟SDK，TapTap SDK\n"" +
                    "" 使用目的：第三方广告：技术上我们将收集一些设备信息保存一些配置。更好的展示广告提高广告价值\n"" +
                    "" 是否与身份关联：收集数据的主要目的是用于“第三方广告-显示广告”，在获得用户授权的情况下，才与设备ID、标识符相关联。\n"" +
                    "" 用于何种追踪目的：收集数据的主要目的是用于“第三方广告-显示广告”。\n"" +
                    ""\n"" +
                    ""2.我们如何使用Cookie等同类技术\n"" +
