...
namespace Watermelon
{
    public class MissionUIPanel : MonoBehaviour
    {
        ...
        public void Unload()
        {
            if (!missionUICases.IsNullOrEmpty())
            {
                foreach (MissionUICase missionUI in missionUICases)
                {
                    missionUI.Unload();
                }
    ...
    public class MissionUnlockUICase : MissionUICase
    {
        ...
        public override void Initialise(MissionUIPanel uiPanel)
        {
            ...
            resourcePool = new Pool(resourcePrefab, resourcePrefab.name, resourcesContainer);
        }
        ...
        {
            CurrencyController.UnsubscribeGlobalCallback(OnCurrencyAmountChanged);
