...
namespace OctoberStudio.Abilities
{
    ...
    public class AbilityManager : MonoBehaviour
    {
        ...
        private StageSaveData StageSaveData => GameEntry.Data.StageSaveData;
        protected virtual void Awake()
        {
            // Usualy the data isn't getting reset only if the Player continues the game after they've closed it without dying
            if (StageSaveData.resetAbilities) StageSaveData.ClearAbilitiesLevels();
        }
        ...
        public virtual void Init(PresetData testingPreset, CharacterData characterData)
        {
            ...
            } else if (!StageSaveData.resetAbilities)
            {
                // The game is being continued after the Player exited it without dying. Loading abilities from the save file

                var savedAbilities = StageSaveData.GetSavedAbilities();
                for(int i = 0; i < savedAbilities.Count; i++)
                {
                    int type = savedAbilities[i];
                    AbilityData data = abilitiesDatabase.GetAbility(type);
                    AddAbility(data, StageSaveData.GetAbilityLevel(type));
                }
        ...
        public virtual void AddAbility(AbilityData abilityData, int level = 0)
        {
                ...
                for (int i = 0; i < abilityData.EvolutionRequirements.Count; i++)
                {
                    ...
                    if (requirement.ShouldRemoveAfterEvolution)
                    {
                        ...
                        if (requiredAbility != null)
                        {
                            ...
                            StageSaveData.RemoveAbility(requiredAbility.AbilityType);
            ...
            StageSaveData.SetAbilityLevel(abilityData.AbilityType, level);
        ...
        {
            return StageSaveData.maxReachedStageId >= ability.UnlockLv;
        }
        ...
        protected virtual List<AbilityData> GetAvailableAbilities()
        {
            ...
            for (int i = 0; i < abilitiesDatabase.AbilitiesCount; i++)
            {
                ...
                //这种能力已经达到了极限。无法进一步升级
                if (StageSaveData.GetAbilityLevel(abilityData.AbilityType) >= abilityData.LevelsCount - 1) continue;
                ...
                if (abilityData.IsEvolution)
                {
                    ...
                    for (int j = 0; j < abilityData.EvolutionRequirements.Count; j++)
                    {
                        ...
                        var requiredAbilityReachedLevel = StageSaveData.GetAbilityLevel(evolutionRequirements.AbilityType) >= evolutionRequirements.RequiredAbilityLevel;
        ...
        {
            return StageSaveData.GetAbilityLevel(abilityType);
        }
        ...
        public virtual void ShowChest()
        {
            ...
            {
                int levelsLeft = ability.LevelsCount - StageSaveData.GetAbilityLevel(ability.AbilityType) - 1;
            ...
            foreach (var ability in selectedAbilities)
            {
                if (IsAbilityAquired(ability.AbilityType))
                {
                    var level = StageSaveData.GetAbilityLevel(ability.AbilityType);

                    if (!ability.IsEndgameAbility) level++;

                    if (level < 0) level = 0;

                    StageSaveData.SetAbilityLevel(ability.AbilityType, level);
        ...
        {
            return StageSaveData.GetAbilityLevel(type);
        }
        ...
        public virtual void RemoveAbilityDev(AbilityData abilityData)
        {
            for (int i = 0; i < aquiredAbilities.Count; i++)
            {
                ...
                if (ability.AbilityData == abilityData)
                {
                    ...
                    StageSaveData.RemoveAbility(abilityData.AbilityType);
        ...
        {
            var level = StageSaveData.GetAbilityLevel(abilityData.AbilityType);
            if (level > 0)
            {
                StageSaveData.SetAbilityLevel(abilityData.AbilityType, level - 1);
