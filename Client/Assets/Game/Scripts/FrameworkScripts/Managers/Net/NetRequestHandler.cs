...
namespace GameScripts
{
    public class NetRequestHandler
    {
        ...
        private void SendMessage<T>(T data) where T : IMessage<T>
        {
            ...
            {
                canSend = NetManager.Instance.IsConnectServer;
            }
            ...
            {
                Debugger.LogError("服务器已断开,无法发送心跳包");
                NetManager.Instance.HandleDisconnected(); // 处理连接断开
        ...
        private List<byte[]> HandleMessage<T>(T data) where T : IMessage<T>
        {
            ...
            message.Token = NetManager.Instance.Token;
            Debugger.Log($"发送消息|网络延时{NetManager.Instance.NetDelay}|{typeof(T).Name}: {message} \n 解析Data：{data}");
        ...
        {
            byte[] tempDatas = data; //NetManager.Instance.handleSubPack.CompressData(data);
        ...
        {
            if (!NetManager.Instance.IsConnectServer) return;
            HeartBeatMsg data = new HeartBeatMsg();
            NetManager.Instance.RefreshLastSendHeartAckTime();
