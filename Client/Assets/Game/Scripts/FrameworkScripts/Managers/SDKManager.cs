...
namespace GameScripts
{
    ...
    public class SDKManager: MonoBehaviour, ISingleton
    {
        ...
        public async Task UploadGameData(string userId, string str)
        {
            ...
            NetManager.Instance.Requset.c2s_request_update_role_info(new Dictionary<string, string>()
            {
            ...
            //         Debugger($"游戏数据 {fileName} 上传成功{result.IsSuccessful()}！");
            //     }
            //     catch (Exception ex)
            //     {
            //         Debugger($"{fileName} 上传状态：<color=red>失败</color>，错误：{ex.Message}");
            //     }
            // }
        }
        ...
        public async UniTask DownloadGameData(string userId)
        {
            ...
            try
            {
                ...
                {
                    Debugger.LogError($"下载成功，保存路径: {tempFilePath}");
