...
namespace GameScripts
{
    public class UIPool
    {
        ...
        public UIPool()
        {
            ...
            UIExpire = GameEntry.ParamsSettings.GetGradeParamData(GameConst.UI_Expire, GameEntry.CurrDeviceGrade);
            ClearInterval =
                GameEntry.ParamsSettings.GetGradeParamData(GameConst.UI_ClearInterval, GameEntry.CurrDeviceGrade);
        }
        ...
        internal void ReleaseAll()
        {
            for (LinkedListNode<UIFormBase> curr = m_UIFormList.First; curr != null;)
            {
                ...
                Debugger.Log(LogCategory.Loader, "释放==" + curr.Value.gameObject);
                Release(curr.Value);
                curr = next;
            }
        }
