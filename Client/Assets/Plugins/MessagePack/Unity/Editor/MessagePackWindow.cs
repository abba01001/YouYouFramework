...
namespace MessagePack.Unity.Editor
{
    ...
    internal static class ProcessHelper
    {
        ...
        public static async Task<string> InvokeProcessStartAsync(string fileName, string arguments)
        {
            ...
            p.Exited += (sender, e) =>
            {
                ...
                var error = p.StandardError.ReadToEnd(); // 读取标准错误流
                p.Dispose();
        
                if (!string.IsNullOrEmpty(error))
                {
                    tcs.TrySetResult($"Error: {error}"); // 如果有错误信息返回
                }
                else
                {
                    tcs.TrySetResult(output); // 返回输出
                }
            };

            return await tcs.Task; // 等待任务完成并返回结果
        }
