...
class Program
{
    ...
    // 定义扩展目录路径
    private static string ClientExtendOutPath => Path.Combine(ClientRoot, "Assets/Game/Scripts/LogicScripts/Data/DataTable/Extend");
    ...
    private static void SaveClientFiles(string fileName, string[,] headArr, byte[] buffer)
    {
        ...
        // 2. 合并生成 Entity 和 DBModel 到一个文件
        if (!Directory.Exists(ClientCodeOutPath)) Directory.CreateDirectory(ClientCodeOutPath);
        string combinedCode = GenerateDataTableCode(fileName, headArr);
        File.WriteAllText(Path.Combine(ClientCodeOutPath, fileName + "DBModel.cs"), combinedCode);

        // 3. 自动生成扩展类 (如果不存在)
        if (!Directory.Exists(ClientExtendOutPath)) Directory.CreateDirectory(ClientExtendOutPath);
        string extFilePath = Path.Combine(ClientExtendOutPath, fileName + "DBModelExt.cs");
        if (!File.Exists(extFilePath))
        {
            File.WriteAllText(extFilePath, GenerateExtensionCode(fileName));
            Console.WriteLine("Auto-generated extension: " + fileName + "DBModelExt.cs");
        }

        Console.WriteLine("Success: " + fileName);
    }

    private static string GenerateDataTableCode(string fileName, string[,] headArr)
    {
        var sb = new StringBuilder();
        AppendFileHeader(sb,fileName);
        sb.AppendLine("using System.Collections.Generic;");
        sb.AppendLine();
        sb.AppendLine("namespace GameScripts");
        sb.AppendLine("{");

        // Entity 部分
        sb.AppendLine($"    // {fileName} Entity");
        sb.AppendLine($"    public partial class {fileName}Entity : ConfigEntityBase");
        sb.AppendLine("    {");
        ...
        // DBModel 部分
        sb.AppendLine($"    public partial class {fileName}DBModel : ConfigBase<{fileName}DBModel, {fileName}Entity>");
        sb.AppendLine("    {");
        sb.AppendLine($"        public override string ConfigName => \"{fileName}\";");
        sb.AppendLine();
    ...
    // 3. 生成扩展类的样板代码
    private static string GenerateExtensionCode(string fileName)
    {
        var sb = new StringBuilder();
        sb.AppendLine("using System.Collections.Generic;");
        sb.AppendLine("using Main;");
        sb.AppendLine();
        sb.AppendLine("namespace GameScripts");
        sb.AppendLine("{");
        sb.AppendLine("    public partial class " + fileName + "DBModel");
        sb.AppendLine("    {");
        sb.AppendLine("        // 在这里添加你的自定义扩展逻辑，例如：");
        sb.AppendLine("        // public Dictionary<int, " + fileName + "Entity> IdByDic;");
        sb.AppendLine();
        sb.AppendLine("        protected override void OnLoadListComple()");
        sb.AppendLine("        {");
        sb.AppendLine("            base.OnLoadListComple();");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
        return sb.ToString();
    ...
    private static void AppendFileHeader(StringBuilder sb, string fileName)
    {
        sb.AppendLine("// ========================================================");
        sb.AppendLine("// 自动生成代码，请勿手动修改！");
        sb.AppendLine($"// 如需拓展请在 Extend 目录下{fileName + "DBModelExt.cs"}对应类进行扩展接口");
        sb.AppendLine("// ========================================================");
        sb.AppendLine();
    }
