    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                worldPos.x +=  sin(waveSample *_WindControl.x)*_WaveControl.x* _WindControl.w*saturate(v.uv.y-_offsetv);
                worldPos.z +=  sin(waveSample *_WindControl.z)*_WaveControl.z* _WindControl.w*saturate(v.uv.y-_offsetv);
                // worldPos.x += sin(_Time.x*_Speed)*saturate(v.uv.y-_offsetv)*_Strength;
